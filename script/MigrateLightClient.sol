...
contract MigrateSP1ICS07Tendermint is Script, Deployments {
    function run() public {
        ...
        SP1ICS07TendermintDeployment[] memory deployments =
            loadSP1ICS07TendermintDeployments(vm, deploymentJson, ics26RouterDeployment.proxy);
        ...
        // Guard against migrating a client to the zero address (e.g. a hand-edited JSON entry with a missing
        // .implementation): migrateClient accepts address(0), after which getClient reverts IBCClientNotFound and
        // a corrective migration is impossible (it calls getClient first). The bash calldata path guards this too.
        vm.assertNotEq(newImplementation, address(0), "implementation is zero; deploy the new SP1 light client first");
        address actualClientAddress = address(ics26Router.getClient(clientIDToMigrate));

        vm.assertNotEq(
            actualClientAddress, newImplementation, "On-chain client address already matches the implementation address"
        );
