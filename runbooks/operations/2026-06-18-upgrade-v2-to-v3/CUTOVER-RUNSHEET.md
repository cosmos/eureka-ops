...
`0x4b46ea82…` on **both** the `cosmoshub-0` and `ledger-mainnet-1` escrows — **2 grants** (`0x64259f72…` is **dropped** by decision 2026-06-18).
...
- [ ] **Shadow-fork dress rehearsal green**, incl. the **exact 8-op fold** (4 core + 2 migrations + 2
  ```
  rate-limiter grants) via `RL_GRANTS="cosmoshub-0:0x4b46ea82…,ledger-mainnet-1:0x4b46ea82…"
  ...
  EOA broadcast) — the production registration path. *(2026-06-17: the then-10-op/4-grant fold rehearsed
  green (≈ 616k gas). **2026-06-18: re-rehearsed as the live 8-op/2-grant fold** (`0x64259f72…` dropped) — the
  exact 8-op bundle executed `status 0x1` for **≈ 523k gas**, safeTxHash matched on-chain, both folded grants
  landed, and the 2-of-5 customizer-Safe `addIBCApp` lands (`getIBCApp(gmpport)` == the ICS27 proxy) with
  `VerifyDeployment` passing.)*
  ```
...
> **DONE — 2026-06-20.** All **8** schedule ops (4 upgrades + 2 migrations + 2 rate-limiter grants), gov-Safe
>
> nonces **18–25**, were collected 4-of-7 and **bulk-executed in ONE Ethereum tx**
>
> `0x19c6cd2ae43cdf494aba9ddf077000e7dd5b04abc7bcabfbe6615345fafeb109` (block **25358768**, mined
>
> **2026-06-20 12:21:59 UTC**) via the canonical MultiSendCallOnly `0x9641d764fc13c8B624c04430C7356C1C7C8102e2`,
>
> gas relayed from a **non-owner** EOA `0x64259f722A0868CCf58A935C61A292cEA9dF035a` (executing a fully-signed
>
> Safe tx is permissionless once the 4-of-7 confirmations are collected). Result: all **8 timelock ops pending**
>
> (delay 259 200 s); gov-Safe nonce **18 → 26**; tx-service shows nonces **18–25 `isExecuted=true`**.
>
> **72 h clock started — ready at `2026-06-23 12:21:59 UTC` (unix 1782217319).** The steps below are the
>
> **record** of what produced this — do not re-run them.

**Take the RATE_LIMITER snapshot now** (escrows are still v2 `AccessControl`):

```bash
python3 scripts/discover-v2-roles.py mainnet 1     # lists live v2 holders (BOTH 0x4b46ea82 & 0x64259f72); re-grant ONLY 0x4b46ea82 — 0x64259f72 DROPPED (decision #9)
```
...
cast decode-calldata 'upgradeToAndCall(address,bytes)' [[ORCA_RAW_HTML_INLINE:%3Cdata%3E]]
cast decode-calldata 'initializeV2(address)' [[ORCA_RAW_HTML_INLINE:%3Cinner%3E]]             # == .accessManager
```
...
**Schedule the rate-limiter re-grants** — **2 pairs**: `0x4b46ea82…` on `cosmoshub-0` and on `ledger-mainnet-1` (one per (client_id, holder); these recipes prompt, so
...
**You (the proposer/coordinator) execute the 8 schedules — signers only sign.** Execute them back-to-back

and in nonce order so all eight 72 h clocks start together; a schedule fired early staggers the delays. In a

4-of-7 Safe the account that adds the 4th confirmation gets an Execute button, so reach the threshold last

(or tell signers to sign-only) — never let a signer execute. The single atomic upgrade is the step-7 execute

below, which you build and execute too.

*(Done 2026-06-20: rather than 8 back-to-back `execTransaction`s, all 8 fully-signed schedules were folded into*

*ONE MultiSendCallOnly bulk tx — every 72 h clock started in the same block, `2026-06-20 12:21:59 UTC`, with no*

*staggering, and a non-owner EOA relayed it.)*
...
**transaction queued at the reserved execute nonce (26)** (would change what executes at that nonce); a gateway
...
1. Confirm the clock reached **`2026-06-23 12:21:59 UTC`** (the 72 h delay elapsed). (Leave the prod relayer **on the old build** — it's still relaying
...
2. Re-run the gates — both must be clean:
  ```bash
   ...
   ETH_RPC=[[ORCA_RAW_HTML_INLINE:%3Crpc%3E]] FROM_BLOCK=22188631 scripts/verify-roots.sh mainnet 1 # must end "ALL TRUST-ROOT CHECKS PASSED"
  ```
...
**Build + propose the atomic MultiSend — the bundle is now concrete (Phase B done).** gov-Safe **nonce 26** →
`to = MultiSendCallOnly 0x9641d764fc13c8B624c04430C7356C1C7C8102e2`, `operation = 1 (DELEGATECALL)`, **8**
timelock `execute(...)` sub-calls (4 upgrades + 2 migrations + 2 rate-limiter grants, selector `0x134008d3`).
The **2 rate-limiter grant execute blobs** captured in Phase B go in `EXTRA_TIMELOCK_OPS`. Expected
**`safeTxHash = 0xe999dee7ac3a0a003383efb2fa45c9b8105ef8c21ab8322dcd9371173f0a637a`** (== on-chain
`getTransactionHash` for nonce 26). The execute is itself a **4-of-7** action: only the gov Safe holds the
timelock's `EXECUTOR_ROLE` (sole grant; the role is not open; the timelock is its own admin).

**You can propose nonce 26 and collect 4-of-7 NOW, during the window.** The `safeTxHash` is fixed and signing
is safe. **The on-chain execute can only succeed at/after `2026-06-23 12:21:59 UTC`:** an early `execTransaction`
**reverts** (sub-ops not ready) and, with `safeTxGas=0`, **does NOT consume nonce 26** — you can retry. So
build/propose **without** `REQUIRE_READY` (the packer's `assert_pending` re-validates each sub-call is *pending*);
add **`REQUIRE_READY=1` only at the real execute** as the final go/no-go (it asserts every sub-op is *ready*, not
just pending).

```bash
# FOUNDRY_ETH_RPC_URL must be set — cast ignores a bare ETH_RPC. Build (no REQUIRE_READY during the window):
EXTRA_TIMELOCK_OPS='0x<rl-grant-1>;0x<rl-grant-2>' \
  just execute-v3-upgrade-multisend 26 cosmoshub-0 ledger-mainnet-1
# prints to/value/operation(=1)/data/safeTxHash — confirm safeTxHash == 0xe999dee7…, then propose at nonce 26:
EXTRA_TIMELOCK_OPS='…' LEDGER=1 MNEMONIC_INDEX=1 \
  just safe-propose 0x9641d764fc13c8B624c04430C7356C1C7C8102e2 <multisend_data> 26 1
# … 4-of-7 sign during the window …
# AT/AFTER 2026-06-23 12:21:59 UTC — rebuild with REQUIRE_READY=1 (final go/no-go: every sub-op must be ready):
EXTRA_TIMELOCK_OPS='…' REQUIRE_READY=1 \
  just execute-v3-upgrade-multisend 26 cosmoshub-0 ledger-mainnet-1
```

4-of-7 sign + execute in the Safe UI / device. **Verify on-device: `to == 0x9641d764…`,**

`**operation == DelegateCall`, `safeTxHash == 0xe999dee7…`.** Execute-round signers use
`[SIGNER-CHECKLIST-EXECUTE.md](SIGNER-CHECKLIST-EXECUTE.md)` and run `bash scripts/signer-verify.sh 26
...
>
> the real gov Safe → `status 1`, gas **523 285**. Post-execute the proxies resolved to the freshly-deployed v3
>
> implementations (ICS20Transfer → `0x1ae0b1071a99166248a78e55547b36d44F5B0790`, ICS26Router →
>
> `0x17fa3A98D0239a399927C7c3CCdE142e08Deb7B5`) and the rate-limiter (role 5, holder `0x4b46ea82…`) was wired on
>
> **both** escrows. This is the live bundle, not a staged variant.
...
> **Dress-rehearsed (2026-06-20, mainnet fork, post-nonce-26-execute state).** On the fork, after the execute
>
> landed: `addIBCApp("gmpport", 0xbebd14A66052d7dc6BDc05e7328E4fEC0a9e3B0e)` via the **2-of-5 customizer Safe**
>
> `0x4b46ea82…` `execTransaction` landed (**step 8**); `initializeV2()` ran on **all three** escrows —
>
> `cosmoshub-0 0x0fA75C2c…`, `ledger-mainnet-1 0xC76944B0…`, `client-4 0x3f36Fd49…` (**step 9**); and
>
> `VerifyDeployment` **PASSED** (**step 11**). Steps 8/9/11 are validated end-to-end against real mainnet state.
>
> **Still required before 8a: deploy the new relayer/prover image.** Its gate PRs are *merged but not yet
>
> deployed* — `solidity-ibc-eureka#1052` (merged 2026-06-18) and
>
> [`ibc-manifests#91`](https://github.com/skip-mev/ibc-manifests/pull/91) (merged 2026-06-17). Build/sync the
>
> image and confirm the `/dev/shm` mount below before rolling.
...
```bash
# data (<ics27Gmp.proxy> is the deployed proxy — rehearsed: 0xbebd14A66052d7dc6BDc05e7328E4fEC0a9e3B0e):
...
cast send [[ORCA_RAW_HTML_INLINE:%3Cescrow_to%3E]] [[ORCA_RAW_HTML_INLINE:%3Cdata%3E]] --rpc-url [[ORCA_RAW_HTML_INLINE:%3Crpc%3E]] --private-key [[ORCA_RAW_HTML_INLINE:%3Cfunded%20EOA%3E]]   # or --ledger
# then confirm each escrow flipped:
cast call [[ORCA_RAW_HTML_INLINE:%3Cescrow%3E]] 'authority()(address)' --rpc-url [[ORCA_RAW_HTML_INLINE:%3Crpc%3E]]                 # == [[ORCA_RAW_HTML_INLINE:%3CaccessManager%3E]]
```
...
tx (`bash scripts/signer-verify.sh <nonce>` for the schedules; `bash scripts/signer-verify.sh 26 --expect-subcalls 8` for the execute) that recomputes the hashes, decodes

what the tx does, and prints PASS/REJECT. Run it from the repo root on the named branch — it finds `COORDINATOR-HASH-TABLE.md` itself and recomputes offline; needs only `cast`.
...
`EXTRA_TIMELOCK_OPS='…' just execute-v3-upgrade-multisend 26 cosmoshub-0 ledger-mainnet-1` → must print
`0xe999dee7…`), or run `bash scripts/signer-verify.sh 26 --expect-subcalls 8`. Reject any step-7 proposal whose
`to != 0x9641d764…`, `operation != DelegateCall`, or `safeTxHash != 0xe999dee7…`.
...
- **Found a problem during the 72 h delay (nothing executed yet):** the v2 system is still fully live (the
  ...
  ```bash
  cast call [[ORCA_RAW_HTML_INLINE:%3Ctimelock%3E]] 'hashOperation(address,uint256,bytes,bytes32,bytes32)(bytes32)' … # the opId
  # propose a Safe CALL to the timelock: cancel(bytes32 id), 4-of-7 sign
  cast calldata 'cancel(bytes32)' [[ORCA_RAW_HTML_INLINE:%3CopId%3E]]
  ```
