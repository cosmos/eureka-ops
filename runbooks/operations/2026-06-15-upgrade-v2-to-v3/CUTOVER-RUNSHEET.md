...
**Build + propose the atomic MultiSend** — the 6 core/migration executes + the **2** rate-limiter grant
...
```bash
EXTRA_TIMELOCK_OPS='0x<rl-1>;0x<rl-2>' REQUIRE_READY=1 \
...
`signer-verify.sh … --expect-subcalls 8` (4 core + 2 migrations + 2 rate-limiter). Atomic + ordered

(ICS20 before ICS26) — if any sub-execute reverts, nothing lands. *(Rehearsed on a mainnet fork: the*

then-10-op bundle executed for ≈ 616k gas; the live 8-op/2-grant bundle is slightly smaller — re-rehearse.)*
